...
describe('UserMessageId', () => {
    ...
    it('should create a new instance with a UUID', () => {
        const id = UserMessageId.create();
        expect(id.getValue()).toBe(VALID_UUID);
        expect(uuidv4).toHaveBeenCalled();
    });

    it('should create from existing string', () => {
        const id = UserMessageId.fromExisting(VALID_UUID);
        expect(id.getValue()).toBe(VALID_UUID);
    });

    it('should throw error for invalid UUID', () => {
        expect(() => UserMessageId.fromExisting('invalid-uuid')).toThrow();
    });
    ...
    it('should be equal to another ID with same value', () => {
        const id1 = UserMessageId.fromExisting(VALID_UUID);
        const id2 = UserMessageId.fromExisting(VALID_UUID);
        expect(id1.equals(id2)).toBe(true);
    });

    it('should not be equal to ID with different value', () => {
        const id1 = UserMessageId.fromExisting(VALID_UUID);
        const id2 = UserMessageId.fromExisting(ANOTHER_UUID);
